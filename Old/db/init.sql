...
-- CORRECTION : Suppression des colonnes générées qui dépendent d'autres colonnes générées
CREATE TABLE IF NOT EXISTS cholera.cas_maladie (
    ...
    -- On garde les colonnes générées pour cas_total et deces_total
    ...
    -- CORRECTION : letalite n'est plus une colonne générée, on la calcule dans les requêtes SQL
    -- On laisse une colonne simple avec un DEFAULT NULL
    letalite NUMERIC(10,2) DEFAULT NULL,
...
-- CORRECTION : Trigger pour mettre à jour letalite automatiquement
CREATE OR REPLACE FUNCTION cholera.update_letalite()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.cas_total > 0 THEN
        NEW.letalite = (NEW.deces_total::NUMERIC / NEW.cas_total) * 100;
    ELSE
        NEW.letalite = NULL;
    END IF;
    RETURN NEW;
...
CREATE TRIGGER cas_maladie_letalite
    BEFORE INSERT OR UPDATE ON cholera.cas_maladie
    FOR EACH ROW EXECUTE FUNCTION cholera.update_letalite();
